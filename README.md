...
- RX copy path — this driver does one `memcpy` per received frame (raw USB buffer → packet pool), the same as Linux, because the RX buffer is reused in-place and must be reposted immediately. The x86 kext achieves zero payload copies via a different strategy (fresh mbuf allocation every cycle + zero-copy mbuf splitting) not directly portable to this driver's model. Separately, RX/TX/ITR pipe buffers are allocated via `IOUSBHostInterface::CreateIOBuffer()`, Apple's documented path for buffers that "will not be bounced to perform DMA operations". Benchmarking against the generic `IOBufferMemoryDescriptor::Create()` allocator showed no measurable throughput difference on Apple Silicon, so this is used as the documented contract rather than a speedup. See `IMPL_PLAN.md` M9.
...
Dext logs are **stream-only**: `os_log` output from a dext (even type Default) is never persisted to the log archive, so `log show` finds nothing after the fact. To capture an event post-hoc, run `sudo log collect --last <N>m --output x.logarchive` promptly (it snapshots the in-memory buffers before they wrap) and query the archive with `log show x.logarchive`. Additionally, log lines emitted in the first moments of system wake are silently dropped — an event that "never logs" on the wake path has not been shown to never happen (see `TESTING.md` Reusable Methodology Notes for the counter technique that proves delivery).
...
**Direction flags are enforced CPU mapping protection for provider-created buffers.** Pipe buffers (TX/RX/ITR) are allocated with `IOUSBHostInterface::CreateIOBuffer()`, Apple's documented allocator for USB DMA buffers ("will not be bounced to perform DMA operations") — but TX must be allocated with `kIOMemoryDirectionOutIn`: a pure `kIOMemoryDirectionOut` interface-created buffer is CPU-read-only and faults when the dext writes the TX descriptor/frame, while a generic `IOBufferMemoryDescriptor::Create()` buffer with the same `Out` direction remains CPU-writable.

That asymmetry is documented XNU policy, not a USB quirk. When a descriptor is mapped into a dext, [`IOMemoryDescriptor::CreateMapping_Impl`](https://github.com/apple-oss-distributions/xnu/blob/xnu-12377.121.6/iokit/Kernel/IOUserServer.cpp#L713-L740) forces `kIOMapReadOnly` for any pure-`kIODirectionOut` descriptor unless the calling task is recorded as the descriptor's creator. Generic dext-side `IOBufferMemoryDescriptor::Create` gets that exemption because its kernel shim [stamps the dext task as creator](https://github.com/apple-oss-distributions/xnu/blob/xnu-12377.121.6/iokit/Kernel/IOUserServer.cpp#L913-L940); `CreateIOBuffer` buffers do not — they are provided by the USB stack rather than created by the dext, so the [direction-derived protection](https://github.com/apple-oss-distributions/xnu/blob/xnu-12377.121.6/iokit/Kernel/IOMemoryDescriptor.cpp#L486-L508) (`Out` → read-only, `In` bit → writable) applies with no exemption. Hence a CPU-written DMA-source buffer must be `kIOMemoryDirectionOutIn` — the intended pattern, per XNU's own development-build warning against writable pure-`Out` mappings.
