...
class ExtensionManager : NSObject, OSSystemExtensionRequestDelegate {
    ...
    func activate() {
        let activationRequest = OSSystemExtensionRequest.activationRequest(forExtensionWithIdentifier: "au.com.jquirke.AQC111", queue: .main)
    ...
    func deactivate() {
        ...
        let activationRequest = OSSystemExtensionRequest.deactivationRequest(forExtensionWithIdentifier: "au.com.jquirke.AQC111", queue: .main)
        activationRequest.delegate = self
        OSSystemExtensionManager.shared.submitRequest(activationRequest)
    }
