...
**Step 1b finding (2026-06-20): `SetSoftwareVlanSupport()` is not load-bearing — removed.** Tested via a real attach/reinstall (not a live toggle — see below): called `SetSoftwareVlanSupport(false)` in `Start()` while deliberately leaving `kIOUserNetworkHWAssistSoftwareVlan` declared in `AQC111_HWASSIST_MASK`/`getFeatureFlags()`, creating an explicit disagreement between the two. VLAN tagging kept working identically — `vlan0` stayed correctly configured, inline tags still arrived, bidirectional ICMP still passed. Conclusion: the dedicated `SetSoftwareVlanSupport()` method call does nothing observable on this DriverKit version (see Step 1c below for what the HWAssist bit itself actually does). The call has been removed entirely from `Start()`. (A live-toggle experiment was tried first, calling `SetSoftwareVlanSupport()` from a userspace tool mid-flow via a temporary `IOUserClient` selector; that showed no effect either, but turned out to be inconclusive on its own — the value is almost certainly consulted once at attach/`RegisterEthernetInterface` time, not re-read live, so a live toggle can't distinguish "doesn't matter" from "matters but only at attach time." The attach-time disagreement test above is what actually settled it. The live-toggle scaffolding was reverted, not kept.)

**Step 1c finding (2026-06-20): what `kIOUserNetworkHWAssistSoftwareVlan` actually does.** Followed up by removing the HWAssist bit itself (not just the method call) from `AQC111_HWASSIST_MASK`, real attach/reinstall, retested against `tools/setup-vlan1234.sh`. Result: VLAN tag delivery/demux *still* worked identically in both directions (`81 00 04 d2` still inline in both `txDrainOne`'s and `OnRxComplete`'s diagnostics) — but `vlan0`'s MTU reverted to the clamped `1496` instead of the full `1500`. Conclusion: the bit controls *only* BSD's `vlan(4)` MTU bookkeeping (does the VLAN sub-interface get the parent's full MTU, or get clamped down by 4 bytes) — it does not gate whether tagging/demuxing happens at all. That's governed entirely by `SFR_VLAN_ID_CONTROL` (hardware RX-strip; see Step 1 above) for RX, and is unconditional OS/software behavior for TX in this Layer-1 model, independent of any DriverKit-level capability declaration. The bit has been restored: it's not load-bearing for tag delivery, but losing 4 bytes of MTU for free traffic is a real, avoidable downside given the `+4` size-check headroom already exists regardless.

**Step 1d finding (2026-06-20): exhaustive — the last combination changes nothing either.** Tested bit removed AND `SetSoftwareVlanSupport(false)` called together (the one permutation not yet tried, to test whether `getVlanTag()`/`setVlanTag()` metadata delivery only kicks in when both signals agree the driver has no software fallback). Real attach/reinstall. Result: identical to every other combination — inline tag still present, `getVlanTag()` still `false`. All four combinations of {bit on/off} × {call true/false/absent} now tested; all four produce identical tag-delivery behavior, with only the bit affecting MTU. This rules out "wrong combination" as the explanation — whatever gates real hardware-VLAN metadata delivery (`kFeatureHardwareVlan` per the SDK doc comment) is not reachable through either of these two mechanisms in any combination. Reverted to the validated state (bit declared, no call) after this test; not a design change.

**Layer 2 status: BLOCKED on Apple, not pursued further.** Two independent reasoning-model consultations, given the exhaustive Step 1d result, both converge on the same conclusion: `kFeatureHardwareVlan` (the capability the SDK doc comment says gates `getVlanTag()`/`setVlanTag()`) was never exposed as a public constant — likely a data-plane/control-plane split where Apple shipped the `IOUserNetworkPacket` methods but withheld the corresponding capability flag, possibly reserved for first-party/PCIe drivers. Treat this as a probable explanation, not a verified fact — neither this project nor an AI model has access to Apple's closed-source implementation to confirm it definitively. The actionable parts that don't depend on that narrative being literally true: this is unreachable from the current public SDK by any combination of mechanism we have access to (confirmed empirically, Step 1d). Posted to Apple Developer Forums asking whether this is reachable at all: https://developer.apple.com/forums/thread/835355 — blocked pending a response, not actively worked. Low priority: VLAN works fully via the software path (Layer 1), and hardware offload is a performance optimization, not a functional gap. Layer 1 remains the supported, validated path; hardware tag insert/strip stays documented as future/blocked work in `README.md`.
...
2. Layer 2 (hardware tag insert/strip) — not started, not currently planned; see closure note above. RX would extract bit 10 + tag from `pd` bits 63:32 (masked to 12-bit VID, matching Linux's explicit `VLAN_VID_MASK`) and call `setVlanTag()`; TX would call `getVlanTag()` and, if present, set TX descriptor bit 29 + tag in bits 63:48 — kept here only as a reference for if this is ever revisited (e.g. after an Apple Feedback/DTS response), not as active work.
