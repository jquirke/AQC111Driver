...
**Known gap, not yet fixed:** the overflow→`AMALL` direction is validated, but the reverse (count dropping back under the threshold) is not, and reasoning through it surfaced a real bug — see "Bug fix plan — AMALL fallback can never be retracted once triggered by address-count overflow" below.
...
### Bug fix plan — AMALL fallback can never be retracted once triggered by address-count overflow (found 2026-06-23, not yet fixed)

**Symptom:** none observed as a live failure yet — found by reasoning through the M6h implementation's design after validating it, before it could cause real harm.
...
**Proposed fix:** track the two triggers in separate ivars (e.g. `allMulticastRequested` for the explicit call, `mcastCountExceeded` for the overflow fallback) instead of a single shared bit, and recompute `SFR_RX_CTL_AMALL` as the OR of both on every change to either. `doSetAllMulticastMode()` updates `allMulticastRequested`; `doSetMulticastAddresses()` updates `mcastCountExceeded` based on the current call's `count`; both then call a shared recompute step. This lets the overflow contribution retract on its own once the count drops back under the threshold, without touching an independently-active explicit request.

**Test plan once fixed:** temporarily lower `AQ_MAX_MCAST_ADDRESSES` again (e.g. back to `5`, same as the M6h validation), trigger the overflow fallback (already confirmed easy — the OS's own standing multicast joins exceed it), then reduce the address count back under the threshold (e.g. leave enough multicast groups, or temporarily raise the threshold instead and confirm a *fresh* `SetMulticastAddresses` call rather than relying on a stale one) and confirm `AMALL` actually clears — re-test the previously-validated non-colliding negative-control address (`01:00:5e:7f:01:02`) and confirm it goes back to being **dropped**, proving the fallback retracted rather than staying stuck on. Also worth a quick check that an explicit `SetAllMulticastModeEnable(true)` independently holds `AMALL` on through an unrelated `SetMulticastAddresses` count drop, to confirm the two triggers genuinely don't clobber each other.
