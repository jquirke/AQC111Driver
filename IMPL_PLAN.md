...
**Open question, not yet resolved:** how much this actually matters in practice. Apple's own `ConnectingANetworkDriver` sample doesn't use `CreateIOBuffer` anywhere (uses the generic allocator, same as this driver today), and the original x86 kext's own RE'd internals literally call its equivalent buffers "DMA/bounce buffer" (`RE_LOG.md:1156`) — bounce-buffering for USB transfers on this hardware seems to have been the normal, working historical approach, not an unusual inefficiency unique to this implementation. Worth an `iperf3` A/B comparison before/after switching to actually confirm a throughput improvement, rather than assuming the doc comment's framing translates to a measurable real-world gain.
