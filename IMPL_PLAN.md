...
### M6e — 802.1Q VLAN support design (baseline tested, implementation not yet started)
...
**Plan, remaining**:
1. Layer 1 (software accommodation): declare `SetSoftwareVlanSupport` override (mirrors `SetWakeOnMagicPacketEnable`'s existing pattern), OR `kIOUserNetworkHWAssistSoftwareVlan` into `AQC111_HWASSIST_MASK`, fix the size-check `+4` gap. Instrument `txDrainOne()` to log the raw bytes/`getVlanTag()` metadata for a VLAN-tagged packet to find out *why* it's not making it to the wire today before assuming the size-check theory is the actual root cause.
