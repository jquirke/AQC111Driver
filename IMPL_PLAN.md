...
A Feedback report has been drafted (in `notes/vlan_re.md`) explaining the documentation gap and asking Apple to either expose the missing capability constant or formally clarify it's unavailable to third-party drivers.

Conclusion unchanged in practice, now on much firmer evidence than the earlier two-reasoning-model consultation: Layer 1 (software VLAN) remains the only supported path. Layer 2 is not worth picking back up unless Apple responds to the Feedback report with something new.
...
### M9 — RX/TX buffer allocation: avoid USB DMA bounce-buffering (planned, performance, not started)

All RX (`RX_SLOTS` buffers), TX, and ITR buffers are currently allocated via the generic `IOBufferMemoryDescriptor::Create(...)`. `IOUSBHostInterface::CreateIOBuffer(IOOptionBits options, uint64_t capacity, IOBufferMemoryDescriptor **buffer)` (USBDriverKit, `LOCALONLY` — no IPC round-trip, unlike the otherwise-equivalent `IOUSBHostDevice::CreateIOBuffer`) is documented to allocate a buffer "optimized for use by the underlying controller hardware" that "will not be bounced to perform DMA operations," implying the generic allocator's buffers may incur an extra CPU copy (bounce buffer) on every USB transfer that this one avoids.

**Fix shape:** swap `IOBufferMemoryDescriptor::Create(kIOMemoryDirectionIn/Out, size, 0, &buf)` for `ivars->interface->CreateIOBuffer(kIOMemoryDirectionIn/Out, size, &buf)` for the RX pool, TX staging buffer, and ITR buffer. (The doc comment names the option constants `kIODirectionIn`/`kIODirectionOut`, which don't exist as symbols anywhere in the DriverKit SDK — almost certainly informal/inexact naming for the real `kIOMemoryDirectionIn`/`kIOMemoryDirectionOut` already used everywhere else in this driver, since `options` is typed as the generic `IOOptionBits`.) `GetAddressRange()` (used by `OnRxComplete` to get a direct CPU pointer into the RX buffer) is a method on `IOBufferMemoryDescriptor` itself regardless of which `Create` variant produced it, so the RX-parsing code shouldn't need to change.

**Open question, not yet resolved:** how much this actually matters in practice. Apple's own `ConnectingANetworkDriver` sample doesn't use `CreateIOBuffer` anywhere (uses the generic allocator, same as this driver today). The x86 kext is not useful precedent either way here — re-disassembled `Tx::init()`/`Rx::init()` (2026-06-23) to check, and the legacy kext's TX/RX buffers are BSD **mbuf** allocations (`_mbuf_alloccluster`/`_mbuf_allocpacket`), a kernel-kext-only mechanism with no DriverKit equivalent; it never had a `Create()` vs `CreateIOBuffer()`-style choice available to it at all (see `RE_LOG.md` "TxRing layout" for the correction — an earlier pass had mislabeled this "DMA/bounce buffer" without disassembly backing that specific claim). Worth an `iperf3` A/B comparison before/after switching to actually confirm a throughput improvement, rather than assuming the doc comment's framing translates to a measurable real-world gain.
