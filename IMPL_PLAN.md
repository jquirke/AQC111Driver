...
### M6e — 802.1Q VLAN support design (Layer 1 fix implemented, TX confirmed working; paused on test-harness issues before full round-trip)
...
**Step 1 result (2026-06-20): Layer 1 fix implemented, TX direction confirmed working.** `SetSoftwareVlanSupport(true)` (note: a concrete base-class method the driver *calls*, not an `override` — the plan above had this backwards) called in `Start()`, `kIOUserNetworkHWAssistSoftwareVlan` added to the hwAssist mask, `AQC111_VLAN_TAG_LEN` (4) added to the TX/RX size checks and buffer sizing. Confirmed via multiple independent signals (MTU correction `1496→1500`, inline tag now present in the driver's own RX'd buffer, remote-side wire capture, remote `vlan1234` RX counter incrementing after a remote-side `rx-vlan-offload` fix) that this driver now correctly tags outbound frames and the remote can correctly demux them. Full positive+negative round-trip not yet completed — every obstacle hit so far traced to test-harness/test-rig state (macOS route ambiguity from `en9`/`vlan0` overlapping `/16`s, a stubborn `permanent` ARP entry, the remote's Realtek/`r8152` adapter's asymmetric `rx`/`tx`-vlan-offload quirks), not driver behavior. Full detail and exact failure-by-failure breakdown in `TESTING.md` "VLAN Support" Step 1. **Uncommitted, on `vlan-support` branch** — paused here per session fatigue with the test harness, to resume next session. RX-side handling (the `OnRxComplete` `+4` fix) still has zero direct test coverage — the reverse-direction (remote→Mac) test that would have exercised it was blocked by the remote's own offload bug before getting that far.

**Plan, remaining**:
1. Fix the test harness properly before further driver-side debugging: give `vlan0` a netmask that doesn't overlap `en9`'s (removing the routing ambiguity at the root, rather than continually re-pinning ARP/routes by hand), and resolve or route around the remote's `tx-vlan-offload` quirk (the `rx-vlan-offload` half is already fixed).
2. Once a clean test rig exists: complete the positive ping round-trip, the negative-control (mismatched VLAN ID) test, and specifically a reverse-direction (remote→Mac) test to finally exercise this driver's `OnRxComplete` size-check fix, which remains unverified.
3. Layer 2 (hardware tag, behind empirical verification, not started): RX extracts bit 10 + tag from `pd` bits 63:32 (masked to 12-bit VID, matching Linux's explicit `VLAN_VID_MASK`) and calls `setVlanTag()`; TX calls `getVlanTag()` and, if present, sets TX descriptor bit 29 + tag in bits 63:48. If `getVlanTag`/`setVlanTag` turn out not to be wired to anything observable, Layer 2 is shelved as "API present, not functionally available," and Layer 1 remains the supported path.

**Verification**: same rigor as RX/TX checksum offload — remote/third-party capture vantage point (not the transmitting Mac — already proven necessary by the Step 0 result, where local capture looked fine while the remote saw nothing), positive test (real `802.1Q` tag on the wire, confirmed via the remote's own parsing, not just IP-level connectivity — done), negative test (mismatched VLAN IDs between the two ends, expect complete silence — same methodology as the bad-checksum negative control — not yet done), and a check that VLAN tagging doesn't perturb the existing checksum-offload classification logging (not yet done). Results in `TESTING.md` "VLAN Support".
